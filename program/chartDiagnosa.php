...
<body style="background: #c4b8e7;">
    ...
    <div class="card ms-3 me-3" style="margin-top: 100px;">
        <div class="card-header text-dark" style="background-color: #A595D6;">
            <h1 style="color: #ffffff; font-family:  'Roboto Mono', monospace; text-align: center; "> CHART DIAGNOSA KLINIK AKIVA</h1>
        </div>
...
<script>
    am5.ready(function() {
        ...
        series.data.setAll([
            <?php
            $query = mysqli_query($konek, "SELECT DISTINCT a FROM diagnosa");

            while ($row = mysqli_fetch_array($query)) {
                $a = $row['a'];

                $data = query("SELECT COUNT(d.a) as jumlah FROM diagnosa d JOIN pasien p JOIN pendaftaran c on p.id_pasien = c.id_pasien AND c.id_pendaftaran = d.id_pendaftaran WHERE a = '$a'")[0];
                $jumlah = $data['jumlah'];
            ?> {
